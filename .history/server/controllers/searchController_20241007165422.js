...
async function scrapeDataFromIkman(keyword) {
    ...
    const productData=await page.evaluate((url)=>{
        const data=Array.from(document.querySelectorAll('.top-ads-container--1jeoq'))
            .map((product)=>(
                {
                    ...
                    listing_URL : product.querySelector('.card-link--3ssYv').href,
                    date_posted: product.querySelector('..updated-time--1DbCk')?.innerText || 'Not '
        ))
